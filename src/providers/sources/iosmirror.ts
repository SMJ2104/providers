...
const baseUrl2 = 'https://m3u8-proxy-orcin.vercel.app/';
...
const universalScraper = async (ctx: ShowScrapeContext | MovieScrapeContext): Promise<SourcererOutput> => {
  ...
  const playlist = `https://m3u8-proxy-orcin.vercel.app/m3u8-proxy?url=${encodeURIComponent(`${baseUrl}${autoFile}`)}&headers=${encodeURIComponent(JSON.stringify({ referer: baseUrl, cookie: makeCookieHeader({ hd: 'on' }) }))}`;
  ctx.progress(90);
